<div align="center">

# Riya Solanki

**Full Stack Developer · Software Engineer**

Bengaluru, Karnataka, India

[riyasolanki2916@gmail.com](mailto:riyasolanki2916@gmail.com) · [LinkedIn](https://www.linkedin.com/in/riya-solanki) · [GitHub](https://github.com/Riya-solanki)

</div>

<br/>

## About

Full-stack developer and software engineer skilled in designing scalable applications, optimizing system performance, and enhancing user experiences. Proficient in front-end and back-end development, UI/UX design, and database management.

<br/>

## Education

**B.Tech in Computer Science** — Amrita Vishwa Vidyapeetam, Bengaluru

Coursework: Data Structures & Algorithms, Operating Systems, Computer Networks, Database Management Systems, Natural Language Processing, Cloud Computing

<br/>

## Skills

| | |
|---|---|
| **Full Stack** | HTML/CSS, JavaScript, Bootstrap, React.js |
| **Languages** | Java, C |
| **Database** | MySQL, MongoDB |
| **Cloud / DevOps** | AWS, Docker, Git/GitHub |
| **Soft Skills** | Leadership, Collaboration, Problem Solving, Time Management |

<br/>

## Certifications

- Academy Cloud Foundation (ACF) — Amazon Web Services, 2026
- AWS Cloud Practitioner — Amazon Web Services, 2026
- JIRA Certification — Atlassian, 2026
- Cloud Computing Foundations — Google, 2025

<br/>

## Achievements

- 280+ problems solved on LeetCode (2023 – Present)
- Interviewed a Software Development Manager on industry trends & engineering culture (2025)

<br/>

## GitHub Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=Riya-solanki&show_icons=true&theme=default&hide_border=true&count_private=true&include_all_commits=true" height="165"/>
<img src="https://streak-stats.demolab.com/?user=Riya-solanki&theme=default&hide_border=true&timezone=Asia/Kolkata" height="165"/>
</div>

<br/>

<div align="center">

*Open to internships, full-time SDE roles, and research opportunities.*

</div>
